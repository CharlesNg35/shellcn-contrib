...
func actions() []plugin.Action {
	return []plugin.Action{
		...
		{ID: "clickhouse.database.drop", Label: "Drop", Icon: icon("trash-2"), RouteID: "clickhouse.database.drop", Params: map[string]string{"database": "${resource.uid}"}, Confirm: true, ConfirmText: "Drop this database? Every table, view, and dictionary it contains is permanently deleted.", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
		...
		{ID: "clickhouse.column.alter", Label: "Modify column", Icon: icon("pencil"), RouteID: "clickhouse.column.alter", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Modify this column? Changing its type rewrites the affected data via a mutation.", OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}},
		{ID: "clickhouse.column.drop", Label: "Drop column", Icon: icon("trash"), RouteID: "clickhouse.column.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this column? Its data is permanently removed.", OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}, Bulk: true},
		...
		{ID: "clickhouse.index.drop", Label: "Drop index", Icon: icon("trash"), RouteID: "clickhouse.index.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this data-skipping index?", OnSuccess: &plugin.ActionSuccess{SelectTab: "indexes"}, Bulk: true},
		...
		{ID: "clickhouse.constraint.drop", Label: "Drop constraint", Icon: icon("trash"), RouteID: "clickhouse.constraint.drop", Params: map[string]string{"database": "${record.database}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this constraint?", OnSuccess: &plugin.ActionSuccess{SelectTab: "constraints"}, Bulk: true},
		{ID: "clickhouse.table.truncate", Label: "Truncate", Icon: icon("trash"), RouteID: "clickhouse.table.truncate", Params: tableParams(), Confirm: true, ConfirmText: "Truncate this table? Every row will be deleted.", Bulk: true},
		{ID: "clickhouse.table.drop", Label: "Drop", Icon: icon("trash-2"), RouteID: "clickhouse.table.drop", Params: tableParams(), Confirm: true, ConfirmText: "Drop this table? The table definition and data will be permanently deleted.", Bulk: true},
		{ID: "clickhouse.view.drop", Label: "Drop", Icon: icon("trash-2"), RouteID: "clickhouse.view.drop", Params: map[string]string{"database": "${resource.namespace}", "view": "${resource.name}"}, Confirm: true, ConfirmText: "Drop this view?", Bulk: true},
		{ID: "clickhouse.process.kill", Label: "Kill query", Icon: icon("circle-stop"), RouteID: "clickhouse.process.kill", Params: map[string]string{"id": "${resource.uid}"}, Confirm: true, ConfirmText: "Kill this running query? It is terminated on the server.", Bulk: true},
		{ID: "clickhouse.mutation.kill", Label: "Kill mutation", Icon: icon("circle-stop"), RouteID: "clickhouse.mutation.kill", Params: map[string]string{"database": "${resource.namespace}", "table": "${resource.scope}", "id": "${resource.name}"}, Confirm: true, ConfirmText: "Kill this mutation? Cancelling a partially applied mutation can leave parts in a mixed state.", Bulk: true},
		...
		{ID: "clickhouse.user.drop", Label: "Drop user", Icon: icon("trash-2"), RouteID: "clickhouse.user.drop", Params: map[string]string{"user": "${resource.name}"}, Confirm: true, ConfirmText: "Drop this user? They can no longer connect.", OnSuccess: &plugin.ActionSuccess{Navigate: plugin.NavigateList}, Bulk: true},
	}
}
