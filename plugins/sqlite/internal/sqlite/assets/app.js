(function () {
  ...
  function buildUI() {
    ...
    refs.file.addEventListener("change", function () { if (refs.file.files && refs.file.files[0]) openFile(refs.file.files[0]); refs.file.value = ""; });
    ...
    refs.download = button("Download", downloadDatabase, "ghost", "Download the current database file"); refs.download.disabled = true;
    refs.dbLabel = el("span", { class: "dbname", text: "No database" });
    var toolbar = el("div", { class: "toolbar" }, [refs.open, refs.new, refs.download, el("span", { class: "spacer" }), refs.dbLabel]);
  ...
  function setupDrop() {
    ...
    refs.app.addEventListener("drop", function (e) { e.preventDefault(); depth = 0; refs.app.classList.remove("dragging"); if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0]) openFile(e.dataTransfer.files[0]); });
  }
  ...
  function schemaGroup(title, names, editable) {
    ...
    names.forEach(function (name) {
      ...
      caret.addEventListener("click", function (e) {
        e.stopPropagation();
        expanded = !expanded; caret.textContent = expanded ? "▾" : "▸"; cols.style.display = expanded ? "block" : "none";
        if (expanded && !cols.dataset.loaded) { columnsOf(name).forEach(function (c) { cols.appendChild(el("div", { class: "col" }, [el("span", { class: "col-name", text: c.name }), el("span", { class: "col-type", text: c.type || "" })])); }); cols.dataset.loaded = "1"; }
      });
  ...
  function listCol(sql) { try { var r = db.exec(sql); return r.length ? r[0].values.map(function (v) { return v[0]; }) : []; } catch (e) { return []; } }
  function columnsOf(name) {
    var out = [];
    try { var r = db.exec("PRAGMA table_info(" + ident(name) + ")"); if (r.length) r[0].values.forEach(function (v) { out.push({ name: v[1], type: v[2], notnull: v[3], dflt: v[4], pk: v[5] }); }); } catch (e) {}
    return out;
  }
  function openView(name) { setEditorValue("SELECT * FROM " + ident(name)); runQuery(); }
  ...
  function isSingleTableSelect(text) { return /^\s*select\s+\*\s+from\s+("?[A-Za-z_][A-Za-z0-9_]*"?)\s*;?\s*$/i.test(text); }
  function singleTableName(text) { var m = text.match(/from\s+"?([A-Za-z_][A-Za-z0-9_]*)"?/i); return m ? m[1] : null; }
  function tableExists(name) { try { return db.exec("SELECT 1 FROM sqlite_master WHERE type='table' AND name='" + name.replace(/'/g, "''") + "'").length > 0; } catch (e) { return false; } }

  function grid(columns, rows, nav, editable, ridOf) {
    ...
    rows.forEach(function (row, r) {
      var tr = el("tr", {});
      var display = editable ? row.slice(1) : row;
      var rid = editable ? ridOf(row) : null;
      var values = null;
      if (editable) {
        values = {};
        table.cols.forEach(function (c, i) { values[c.name] = display[i]; });
        var cb = el("input", { type: "checkbox" });
        cb.addEventListener("change", function () { if (cb.checked) selection[rid] = { rid: rid, values: values }; else delete selection[rid]; tr.classList.toggle("sel", cb.checked); refreshEditDel(); });
        tr.appendChild(el("td", { class: "selcol" }, [cb]));
      }
      tr.appendChild(el("td", { class: "rownum", text: String(nav.start + r + 1) }));
      display.forEach(function (cell) { tr.appendChild(cellNode(cell)); });
      if (editable) tr.addEventListener("dblclick", function () { openRowForm("edit", { rid: rid, values: values }); });
  ...
  function refreshEditDel() {
    var n = Object.keys(selection).length;
    if (refs.editBtn) refs.editBtn.disabled = n !== 1;
    if (refs.delBtn) refs.delBtn.disabled = n === 0;
  }
  ...
  function saveRow(kind, inputs, existing, overlay) {
    try {
      if (kind === "edit") {
  ...
  async function boot() {
    ...
    try {
      await initEngine();
      refs.open.disabled = false; refs.new.disabled = false;
      welcome();
      setStatus("Ready — open a database to begin", "ok");
    } catch (e) { renderError("Engine failed to load: " + msg(e)); setStatus("Engine failed to load", "error"); }
  }

  function styleTag() {
    var css = [
      ":root{--accent:#0ea5e9}",
      'body[data-theme="dark"]{--bg:#0b1220;--panel:#0f1a2e;--panel2:#16233b;--head:#152238;--border:#243449;--text:#e6edf6;--muted:#8598b3;--danger:#f87171;--ok:#34d399;--sel:rgba(14,165,233,.16)}',
      'body[data-theme="light"]{--bg:#f6f8fb;--panel:#ffffff;--panel2:#eef2f7;--head:#f1f5f9;--border:#dbe2ec;--text:#0f172a;--muted:#64748b;--danger:#dc2626;--ok:#059669;--sel:rgba(14,165,233,.12)}',
      ...
      ".grid tbody tr.sel td{background:var(--sel)}",
