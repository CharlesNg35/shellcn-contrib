...
func actions() []plugin.Action {
	return []plugin.Action{
		...
		{ID: "cockroachdb.database.drop", Label: "Drop database", Icon: icon("trash-2"), RouteID: "cockroachdb.database.drop", Params: map[string]string{"database": "${resource.uid}"}, Confirm: true, ConfirmText: "Drop this database? All of its schemas and data will be permanently deleted.", Bulk: true},
		{ID: "cockroachdb.schema.create", Label: "Create schema", Icon: icon("folder-plus"), RouteID: "cockroachdb.schema.create"},
		{ID: "cockroachdb.schema.drop", Label: "Drop schema", Icon: icon("trash-2"), RouteID: "cockroachdb.schema.drop", Params: map[string]string{"schema": "${resource.uid}"}, Confirm: true, ConfirmText: "Drop this schema? It must be empty.", Bulk: true},
		...
		{ID: "cockroachdb.column.rename", Label: "Rename column", Icon: icon("pencil"), RouteID: "cockroachdb.column.rename", Params: map[string]string{"schema": "${record.schema}", "table": "${record.table}", "name": "${record.name}"}, OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}},
		{ID: "cockroachdb.column.alter", Label: "Change type", Icon: icon("wand-2"), RouteID: "cockroachdb.column.alter", Params: map[string]string{"schema": "${record.schema}", "table": "${record.table}", "name": "${record.name}"}, OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}},
		{ID: "cockroachdb.column.drop", Label: "Drop column", Icon: icon("trash"), RouteID: "cockroachdb.column.drop", Params: map[string]string{"schema": "${record.schema}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this column? Its data is permanently removed.", OnSuccess: &plugin.ActionSuccess{SelectTab: "columns"}, Bulk: true},
		...
		{ID: "cockroachdb.constraint.drop", Label: "Drop constraint", Icon: icon("trash"), RouteID: "cockroachdb.constraint.drop", Params: map[string]string{"schema": "${record.schema}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this constraint?", OnSuccess: &plugin.ActionSuccess{SelectTab: "constraints"}, Bulk: true},
		...
		{ID: "cockroachdb.index.drop", Label: "Drop index", Icon: icon("trash"), RouteID: "cockroachdb.index.drop", Params: map[string]string{"schema": "${record.schema}", "table": "${record.table}", "name": "${record.name}"}, Confirm: true, ConfirmText: "Drop this index?", OnSuccess: &plugin.ActionSuccess{SelectTab: "indexes"}, Bulk: true},
		{ID: "cockroachdb.table.truncate", Label: "Truncate", Icon: icon("trash"), RouteID: "cockroachdb.table.truncate", Params: tableParams(), Confirm: true, ConfirmText: "Truncate this table? Every row will be deleted.", Bulk: true},
		{ID: "cockroachdb.table.drop", Label: "Drop", Icon: icon("trash-2"), RouteID: "cockroachdb.table.drop", Params: tableParams(), Confirm: true, ConfirmText: "Drop this table? The table definition and data will be permanently deleted.", Bulk: true},
		{ID: "cockroachdb.view.drop", Label: "Drop", Icon: icon("trash-2"), RouteID: "cockroachdb.view.drop", Params: map[string]string{"schema": "${resource.namespace}", "view": "${resource.name}"}, Confirm: true, ConfirmText: "Drop this view?", Bulk: true},
		{ID: "cockroachdb.session.cancel", Label: "Cancel session", Icon: icon("circle-stop"), RouteID: "cockroachdb.session.cancel", Params: map[string]string{"id": "${resource.uid}"}, Confirm: true, ConfirmText: "Cancel this session? Its active query is stopped and the session ends.", Bulk: true},
		{ID: "cockroachdb.query.cancel.id", Label: "Cancel query", Icon: icon("circle-stop"), RouteID: "cockroachdb.query.cancel.id", Params: map[string]string{"id": "${resource.uid}"}, Confirm: true, ConfirmText: "Cancel this query? It will be stopped.", Bulk: true},
		...
		{ID: "cockroachdb.user.drop", Label: "Drop user", Icon: icon("user-minus"), RouteID: "cockroachdb.user.drop", Params: map[string]string{"user": "${resource.uid}"}, Confirm: true, ConfirmText: "Drop this user? They lose access to the cluster.", Bulk: true},
	}
}
