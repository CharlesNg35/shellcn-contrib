# ShellCN Cockroachdb plugin

External ShellCN plugin for Cockroachdb.
...
```sh
CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o shellcn-plugin-cockroachdb ./cmd/shellcn-plugin-cockroachdb
```
