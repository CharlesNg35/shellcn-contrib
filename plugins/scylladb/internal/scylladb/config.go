...
func configSchema() plugin.Schema {
	...
	return plugin.Schema{Groups: []plugin.Group{
		{Name: "Cluster", Fields: []plugin.Field{
			{Key: "hosts", Label: "Contact points", Type: plugin.FieldTextarea, Required: true, Placeholder: "10.0.0.10\n10.0.0.11", Help: "Comma or newline separated ScyllaDB contact points."},
			{Key: "port", Label: "CQL port", Type: plugin.FieldNumber, Required: true, Default: defaultPort, Validators: []plugin.Validator{{Type: plugin.ValidatorMin, Value: 1}, {Type: plugin.ValidatorMax, Value: 65535}}},
			{Key: "keyspace", Label: "Default keyspace", Type: plugin.FieldText, Placeholder: "app"},
			{Key: "local_dc", Label: "Local datacenter", Type: plugin.FieldText, Help: "Enables datacenter-aware, token-aware, shard-aware routing when set."},
		...
		{Name: "Authentication", Fields: []plugin.Field{
			...
			}, VisibleWhen: &credentialAuth, Help: "Reusable ScyllaDB password. The credential identity can also supply the username."},
		...
		{Name: "Shard awareness", Fields: []plugin.Field{
			{Key: "connections_per_host", Label: "Connections per host", Type: plugin.FieldNumber, Default: defaultNumConns, Validators: []plugin.Validator{{Type: plugin.ValidatorMin, Value: 1}, {Type: plugin.ValidatorMax, Value: 64}}, Help: "Set at least one connection per shard so every core is reachable without the shard-aware port."},
			{Key: "shard_aware_port", Label: "Use the shard-aware port", Type: plugin.FieldToggle, Help: "Connects to the node's shard-aware port (CQL port + 10000) to land one connection on every shard deterministically. The gateway egress policy must allow that extra port."},
		}},
...
func parseOptions(cfg plugin.ConnectConfig) (options, error) {
	...
	return options{
		...
		ShardAwarePort:    sqldb.BoolValue(cfg.Config["shard_aware_port"], false),
		TLSMode:           tlsMode,
		...
		PageSize:          clampInt(cfg, "page_size", defaultPageSize, plugin.MaxPageLimit),
		RowLimit:          clampInt(cfg, "row_limit", defaultRowLimit, plugin.MaxPageLimit),
		NumConns:          clampInt(cfg, "connections_per_host", defaultNumConns, 64),
...
func clusterConfig(opts options, netTransport plugin.NetTransport) (*gocql.ClusterConfig, error) {
	...
	// The shard-aware port is a second port on every node; gateway egress is
	// scoped to the configured CQL port, so it stays opt-in.
	cluster.DisableShardAwarePort = !opts.ShardAwarePort
...
func clampInt(cfg plugin.ConnectConfig, key string, fallback, maxValue int) int {
