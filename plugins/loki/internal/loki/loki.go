...
func (Plugin) Manifest() plugin.Manifest {
	return plugin.Manifest{
		...
		Resources:           resources(),
		Actions:             actions(),
		Streams:             []plugin.Stream{{ID: rid("query"), Kind: plugin.StreamQuery, RouteID: rid("query")}},
	}
}
...
func parseAuth(cfg plugin.ConnectConfig) (searchrest.Auth, error) {
	...
	case "stored_bearer":
		return tokenAuth(cfg, bearerCredentialField, "Loki bearer credentials require a token")
	case "stored_api_token":
		return tokenAuth(cfg, apiTokenField, "Loki API token credentials require a token")
	default:
