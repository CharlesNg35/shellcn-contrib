// Package jaeger implements the Jaeger protocol plugin.
package jaeger
...
func (Plugin) Manifest() plugin.Manifest {
	return plugin.Manifest{
		...
		Version:             "0.1.0",
		Title:               "Jaeger",
		Description:         "Jaeger tracing cockpit with services, operations, trace search, and trace detail inspection.",
		Icon:                plugin.Icon{Type: plugin.IconSVG, Value: iconSVG},
		Category:            plugin.CategoryObservability,
		Config:              configSchema(),
		Capabilities:        []plugin.Capability{"traces", "services", "operations"},
...
func parseAuth(cfg plugin.ConnectConfig) (searchrest.Auth, error) {
	...
	case "stored_basic":
		...
		if username == "" {
			return searchrest.Auth{}, fmt.Errorf("%w: Jaeger basic credentials require a username", plugin.ErrInvalidInput)
		}
		return basicAuth(username, dbcred.ResolvedSecret(cfg, basicCredentialField)), nil
	case "stored_bearer":
		return tokenAuth(cfg, bearerCredentialField, "Jaeger bearer credentials require a token")
	case "stored_api_token":
		return tokenAuth(cfg, apiTokenField, "Jaeger API token credentials require a token")
	default:
		return searchrest.Auth{}, fmt.Errorf("%w: unsupported authentication mode %q", plugin.ErrInvalidInput, auth)
	}
}

func tokenAuth(cfg plugin.ConnectConfig, field string, message string) (searchrest.Auth, error) {
	token := dbcred.ResolvedSecret(cfg, field)
	if token == "" {
		return searchrest.Auth{}, fmt.Errorf("%w: %s", plugin.ErrInvalidInput, message)
	}
	return searchrest.Auth{Header: "Authorization", Value: "Bearer " + token}, nil
}
...
func configSchema() plugin.Schema {
	return plugin.Schema{Groups: []plugin.Group{
		{Name: "Server", Fields: []plugin.Field{
			{Key: "endpoint", Label: "Endpoint", Type: plugin.FieldText, Required: true, Default: "http://localhost:16686", Placeholder: "https://jaeger-query.example.internal"},
		}},
		{Name: "Authentication", Fields: []plugin.Field{
			{Key: "auth", Label: "Authentication", Type: plugin.FieldSelect, Required: true, Default: "none", Options: []plugin.Option{
				...
				{Label: "Stored API token", Value: "stored_api_token"},
			}},
		...
		{Name: "Query", Fields: []plugin.Field{
			{Key: "timeout", Label: "Request timeout", Type: plugin.FieldDuration, Default: "15s"},
			{Key: "page_limit", Label: "Trace search limit", Type: plugin.FieldNumber, Default: defaultPageLimit, Validators: []plugin.Validator{{Type: plugin.ValidatorMin, Value: 1}, {Type: plugin.ValidatorMax, Value: plugin.MaxPageLimit}}},
		}},
	}}
}
