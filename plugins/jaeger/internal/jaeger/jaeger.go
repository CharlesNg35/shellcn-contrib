...
func parseAuth(cfg plugin.ConnectConfig) (searchrest.Auth, error) {
	...
	case "credential":
		...
		case plugin.CredentialBearerToken, plugin.CredentialAPIToken:
			...
			if token == "" {
				return searchrest.Auth{}, fmt.Errorf("%w: Jaeger token credentials require a token", plugin.ErrInvalidInput)
			}
			return searchrest.Auth{Header: "Authorization", Value: "Bearer " + token}, nil
		case "":
			return searchrest.Auth{}, nil
		default:
			return searchrest.Auth{}, fmt.Errorf("%w: Jaeger stored credentials must be basic auth or token credentials", plugin.ErrInvalidInput)
		}
	default:
