package notebook

import "github.com/charlesng35/shellcn/sdk/plugin"

const protocolName = "notebook"

const IconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path d="M109.766 7.281a7.691 7.691 0 01-1.09 4.282 7.583 7.583 0 01-3.262 2.949 7.49 7.49 0 01-4.34.62 7.525 7.525 0 01-3.953-1.913A7.642 7.642 0 0195.137 5a7.606 7.606 0 012.629-3.531 7.509 7.509 0 014.136-1.461 7.51 7.51 0 015.422 1.996 7.627 7.627 0 012.438 5.273zm0 0" fill="#767677"/><path d="M65.758 96.79c-20.098 0-37.649-7.364-46.766-18.267a49.95 49.95 0 0018.102 24.254 49.251 49.251 0 0028.676 9.215 49.279 49.279 0 0028.675-9.215 49.917 49.917 0 0018.094-24.254C103.406 89.426 85.855 96.79 65.758 96.79zm0 0M65.75 25.883c20.098 0 37.652 7.367 46.766 18.265a49.95 49.95 0 00-18.102-24.253 49.27 49.27 0 00-28.672-9.22 49.27 49.27 0 00-28.672 9.22A49.909 49.909 0 0018.97 44.148C28.102 33.27 45.652 25.883 65.75 25.883zm0 0" fill="#f37726"/><path d="M38.164 117.984a9.671 9.671 0 01-1.371 5.399 9.5 9.5 0 01-9.59 4.504 9.405 9.405 0 01-4.98-2.418 9.671 9.671 0 01-2.809-4.797 9.73 9.73 0 01.313-5.567 9.624 9.624 0 013.328-4.453 9.466 9.466 0 0112.043.688 9.63 9.63 0 013.066 6.648zm0 0" fill="#989798"/><path d="M21.285 23.418a5.53 5.53 0 01-3.14-.816 5.627 5.627 0 01-2.618-5.672 5.612 5.612 0 011.407-2.95 5.593 5.593 0 012.789-1.664 5.46 5.46 0 013.238.184 5.539 5.539 0 012.586 1.969 5.66 5.66 0 01-.399 7.129 5.557 5.557 0 01-3.867 1.82zm0 0" fill="#6f7070"/></svg>`

type Notebook struct{}

func New() Notebook { return Notebook{} }

func (Notebook) Manifest() plugin.Manifest {
	return plugin.Manifest{
		APIVersion:  plugin.CurrentAPIVersion,
		Name:        protocolName,
		Version:     "0.1.0",
		Title:       "Notebook",
		Description: "Agent-hosted Docker sandbox running JupyterLab through a connection-scoped web proxy.",
		Icon:        plugin.Icon{Type: plugin.IconSVG, Value: IconSvg},
		Category:    plugin.CategoryDevOps,
		Config:      configSchema(),
		Capabilities: []plugin.Capability{
			"notebook",
			"python",
			"web_proxy",
		},
		SupportedTransports: []plugin.Transport{plugin.TransportAgent},
		Agent: &plugin.AgentProfile{
			Proxy: plugin.ProxyTarget{Mode: plugin.AgentUnix, Address: agentDockerSocket, Risk: plugin.RiskPrivileged, Forward: true},
			Install: []plugin.InstallArtifact{
				{
					Label:      "Docker",
					Kind:       "docker-run",
					ConnectURL: plugin.ArtifactConnectURL{LocalhostHost: "host.docker.internal"},
					Template: "docker run --rm --name " + plugin.AgentBinary + "-notebook --network host " +
						"{{if .LocalhostHostRequired}}--add-host={{.LocalhostHost}}:host-gateway {{end}}" +
						`--group-add "$(stat -c '%g' /var/run/docker.sock)" ` +
						"-e SHELLCN_CONNECT_URL={{shellquote .ConnectURL}} " +
						"{{if .Insecure}}-e SHELLCN_INSECURE=1 {{end}}" +
						"-e SHELLCN_ENROLL_TOKEN={{shellquote .Token}} " +
						"-v {{shellquote \"/var/run/docker.sock:/var/docker/docker.sock\"}} " +
						"{{shellquote .Image}}",
				},
				{
					Label:    "Docker Compose",
					Kind:     "docker-compose",
					Filename: "shellcn-notebook-agent.compose.yml",
					Content:  composeContent,
				},
			},
		},
		Layout: plugin.LayoutSingle,
		Tabs: []plugin.Panel{{
			Key:   "notebook",
			Label: "Notebook",
			Icon:  icon("notebook-tabs"),
			Type:  plugin.PanelWebProxy,
			Config: plugin.WebProxyConfig{
				Path:          "/lab",
				InlineToolbar: boolPtr(false),
				Capabilities: []plugin.WebProxyCapability{
					plugin.WebProxyCapabilityClipboard,
					plugin.WebProxyCapabilityDownloads,
					plugin.WebProxyCapabilityFullscreen,
					plugin.WebProxyCapabilitySameOrigin,
				},
				AriaLabel:    "Notebook workspace",
				Instructions: "Run notebooks through the connection-scoped JupyterLab proxy.",
			},
		}},
	}
}

func (Notebook) Routes() []plugin.Route { return nil }

func configSchema() plugin.Schema {
	return plugin.Schema{Groups: []plugin.Group{
		{Name: "Sandbox", Fields: []plugin.Field{
			{Key: "sandbox", Label: "Sandbox", Type: plugin.FieldSelect, Default: sandboxDocker, Help: "Docker is the supported sandbox runtime.", Options: []plugin.Option{
				{Label: "Docker", Value: sandboxDocker},
			}},
		}},
		{Name: "Notebook runtime", Fields: []plugin.Field{
			{Key: "image", Label: "Notebook image", Type: plugin.FieldText, Default: defaultNotebookImage, Help: "Docker image used for JupyterLab sessions."},
		}},
	}}
}

func icon(name string) plugin.Icon {
	return plugin.Icon{Type: plugin.IconLucide, Value: name}
}

func boolPtr(v bool) *bool {
	return &v
}

const composeContent = `services:
  shellcn-agent:
    image: "{{.Image}}"
    container_name: shellcn-agent-notebook
    restart: unless-stopped
    network_mode: host
    security_opt:
      - no-new-privileges:true
    cap_drop:
      - ALL
    read_only: true
    tmpfs:
      - /tmp
    environment:
      SHELLCN_CONNECT_URL: "{{.GatewayConnectURL}}"
      SHELLCN_ENROLL_TOKEN: "{{.Token}}"
{{if .Insecure}}      SHELLCN_INSECURE: "1"
{{end}}    volumes:
      - "/var/run/docker.sock:/var/docker/docker.sock"
`
